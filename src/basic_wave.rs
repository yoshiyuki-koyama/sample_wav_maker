...
pub fn create_sin_wave(wave_format: &WaveFormat, amplitude: f64, frequency: f64, pi_phase: f64, len: usize) -> Vec<Vec<f64>> {
    ...
    for _ in 0..wave_format.channel {
        ...
        for idx in 0..len {
            let level = (2.0 * std::f64::consts::PI * frequency * (idx as f64) / wave_format.sampling_rate as f64
                + pi_phase * std::f64::consts::PI)
                .sin()
                * amp_coef;
            data_vec.push(level);
        }
...
pub fn create_square_wave(wave_format: &WaveFormat, amplitude: f64, frequency: f64, pi_phase: f64, len: usize) -> Vec<Vec<f64>> {
    ...
    for _ in 0..wave_format.channel {
        ...
        for idx in 0..len {
            let level =
                if ((2.0 * frequency * (idx as f64) / wave_format.sampling_rate as f64 + pi_phase).floor() as usize % 2) == 0 {
                    amp_coef
                } else {
                    -amp_coef
                };
